...
static uint16_t readPROMWord(int8_t reg)
{
    ...
    i2c_result result = i2cMasterRegisterRead(hBarom, addr, i2cBuff, 2);
...
static void reset()
{
    ...
    i2c_result result = i2cMasterWrite(hBarom, i2cBuff, 1);
    wait(3);
}
...
static float read_adc(uint8_t conv_cmd, uint8_t osr)
{
    ...
    i2c_result result = i2cMasterRegisterRead(hBarom, MS5611_ADC_ADDR, i2cBuff, 3);
    if (result == i2cRes_noerror)
    {
