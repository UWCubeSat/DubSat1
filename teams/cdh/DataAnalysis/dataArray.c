...
void printArray_int16_t(uint16_t handle) {
	buffer_int16_t *theirBuffer = &bufferContainer_int16_t[handle];
	for (int i = 0; i < theirBuffer->maxSize; i++) {
		int16_t number = theirBuffer->bufferPt[i];
		printf("Position %d: %u\n", i, number);
	}
}

void printAllStats_int16_t(uint16_t handle) {
	buffer_int16_t *theirBuffer = &bufferContainer_int16_t[handle];
	NEXT_int16_t sum = getSum_int16_t(handle);
	printf("Sum: %u\n", sum);
	int16_t min = theirBuffer->min;
	int16_t max = theirBuffer->max;
	printf("Min: %hu, Max: %hu\n", min, max);
	int16_t avg = getAvg_int16_t(handle);
	printf("Avg: %u\n", avg);
}
